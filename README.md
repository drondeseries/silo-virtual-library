...
Silo Virtual Library is a zero-storage playback plugin for Silo Server. Silo stores lightweight virtual media references such as `virtual://movie/tt0133093`; no video files, manifests, or segments are persisted by the plugin. At playback time, the plugin asks the configured streaming provider instance for a current upstream streaming URL.
...
1. A Silo library item registered through the authenticated plugin control plane points at an `virtual://` virtual path.
